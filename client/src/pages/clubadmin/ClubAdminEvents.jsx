...
export default function ClubAdminEvents() {
  ...
  const [form, setForm] = useState({
    title: '', description: '', date: '', location: '', capacity: '', members_only: false
  })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ title: '', description: '', date: '', location: '', capacity: '', members_only: false })
      setShowForm(false)
      fetchEvents()
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 flex items-center justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full border ${ new Date(event.date) < new Date() ? 'bg-gray-500/10 border-gray-500/30 text-gray-400' : event.status === 'upcoming' ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-gray-500/10 border-gray-500/30 text-gray-400' }`}> {new Date(event.date) < new Date() ? 'past' : event.status} </span>
                </div>
            ...
            {editingEvent === event.id && (
              <div className="bg-gray-800 border border-gray-700 rounded-2xl p-5 mt-2">
                ...
                <form onSubmit={handleEdit} className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm text-gray-300 mb-1">Date & Time</label>
                    <input type="datetime-local" value={editForm.date} onChange={e => setEditForm({...editForm, date: e.target.value})} min={new Date().toISOString().slice(0, 16)} required className="w-full bg-gray-900 border border-gray-700 text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-purple-500" />
                  </div>
                  <div>
                    <label className="block text-sm text-gray-300 mb-1">Location</label>
